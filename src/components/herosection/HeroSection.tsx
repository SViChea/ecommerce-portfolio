...
export default function HeroSection() {
  ...
  const italic = 'normal'
  const color = '#F27C7A'
  const fontFamily = "Monserrat"
  return (
    <main className='bg-[url(/images/herobackground.jpg)] bg-cover w-fill h-[500px] rounded-2xl mt-5 p-20 mx-[120px]'>
        <div className='flex flex-col gap-2'>
          <p className='font-bold w-[700px] text-white leading-[1]' style={{fontSize: `${size}px`, fontWeight: `${bold}`, fontStyle: `${italic}`, color: `${color}`, fontFamily: `${fontFamily}`}}>Fresh Vegetables Big discount</p>
